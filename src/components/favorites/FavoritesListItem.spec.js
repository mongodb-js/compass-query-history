...
import { FavoritesListItem } from 'components/favorites';
import Query from 'components/query';
import { Card, CardHeader, CardBody } from 'components/card';

describe('FavoritesListItem [Component]', () => {
  const date = new Date();
  const favorite = {
    getAttributes: () => {
      return {
  ...
  beforeEach((done) => {
    ...
    component = shallow(<FavoritesListItem model={favorite} actions={actions} />);
    done();
  });
